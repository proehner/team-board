...
import { Users, Plus, Pencil, Trash2, ShieldCheck, User, Loader2, Key, Monitor, UsersRound, Tag } from 'lucide-react'
import { adminApi, teamsApi, ticketCategoriesApi } from '@/api/client'
import type { AdminUser, Software, Team, TicketCategory } from '@/types'
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'users' | 'software' | 'teams' | 'ticketCategories'>('users')
  ...
  async function confirmDeleteCat() {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : t('admin.errorDeleting'))
    } finally {
  ...
  useEffect(() => { loadUsers(); loadTeams(); loadTicketCats() }, [])
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          {ticketCats.length === 0 ? (
            <div className="py-16 text-center text-slate-400">
              ...
              <p className="text-sm">{t('admin.ticketCategories.empty')}</p>
            </div>
          ) : (
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {ticketCats.map((cat) => (
                  <tr key={cat.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-xs font-mono text-slate-500 dark:text-slate-400">{cat.color}</span>
                    </td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1 justify-end">
                        <button
                          onClick={() => { setEditCat(cat); setCatForm({ name: cat.name, color: cat.color }); setCatFormError(''); setCatModalOpen(true) }}
                          className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950 rounded-lg transition-colors"
                        >
                          <Pencil className="w-4 h-4" />
                        </button>
      ...
      {/* Ticket Category Create / Edit Modal */}
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="form-input w-full"
              placeholder={t('admin.ticketCategories.namePlaceholder')}
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span
                ...
                style={{ backgroundColor: catForm.color + '22', color: catForm.color }}
              >
      ...
      {/* Ticket Category Delete Confirm */}
      <ConfirmDialog
        isOpen={!!catDeleteTarget}
        title={t('admin.ticketCategories.delete')}
        message={t('admin.ticketCategories.deleteConfirm', { name: catDeleteTarget?.name ?? '' })}
        confirmLabel={t('common.delete')}
        variant="danger"
